...
public:                     // area aberta
    Array(int n, T initial) // construtor // define as variaveis
    {
    ...
    {
        ofstream afile; // output file //fprintf??
        afile.open(fileName);
        afile << length << endl;
        for (int x = 0; x < length; x = x + 1)
        {
    ...
    {
        ifstream afile; // input file //fscanf??
        int n = 0;
        afile.open(fileName);
        afile >> n;
        if (n <= 0)
        ...
        else
        {
            ...
            length = n;
            // reservar area
            data = new T[n]; // alocacao de memoria dinamica
            // ler dados
            for (int x = 0; x < length; x = x + 1)
            {
                afile >> data[x];
