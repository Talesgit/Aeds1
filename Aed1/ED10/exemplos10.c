...
typedef int_Array *ref_int_Array; // SEMPRE QUE CHAMAR ref_int.. SERA UM PONTEIRO DO TIPO int_Array
...
{
    // definir dado
    int_Array array; // VARIAVEL DO TIPO int_Array??
    // montar arranjo em estrutura
    array.length = 5; // ATRIBUIR 5 A VAR lenght DA STRUCT
    // ARRAY.LENTH * == NO CASO CRIOU UM VETOR TAMANHO 5
    array.data = (ints)malloc(array.length * sizeof(int)); // CRIOU UM VETOR NA VAR DATA?
    // testar a existência de dados
    if (array.data)
    { // CASO TENHA MALLOCADO, POPULOU O VETOR
        array.data[0] = 1;
        array.data[1] = 2;
        array.data[2] = 3;
        array.data[3] = 4;
    ...
    printIntArray(array); // ENVIOU PARA IMPRESSAO
    // reciclar o espaco
    if (array.data) // LIBEROU O ESPAÇO
    {
        free(array.data);
    } // fim se
...
int_Array IO_readintArray() // CRIA FUNCAO DO TIPO int_Array
{
    // definir dados locais
    // char* text = IO_new_chars(STR_SIZE); // CRIOU UM VETOR??
    char *text = (char *)malloc(STR_SIZE + 1 * sizeof(char));
...
int_Array fIO_readintArray(chars fileName)
{
    ...
    if (arquivo)
    {
        ...
        {
            // reservar espaco
            array.data = IO_new_ints(array.length); // == array.data = (int*)malloc(array.lenth * sizeof(int))
            // testar a existência
            if (array.data)
            {
                // ler e guardar valores em arranjo
...
int main()
{
    printf("ED10 - EXEMPLOS - Tales Rocha Moreira");
